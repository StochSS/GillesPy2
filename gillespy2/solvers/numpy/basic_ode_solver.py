...
class BasicODESolver(GillesPySolver):
    ...
    def __run(self, model, t=20, number_of_trajectories=1, dt=0.05, timeout=None,
        ...
        sol = solve_ivp(BasicODESolver.__f, [timeline[0], timeline[-1]], y0, 
                        args=(propensity, sm, propensities), t_eval=timeline,
                        method=integrator, **integrator_options)
        entry_count = 0
        for i in range(len(timeline)):
            result[i][1:] = sol.y[:number_species, i]
        if show_labels:
            results_as_dict = {
                'time': timeline
            }
