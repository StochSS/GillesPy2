...
GILLESPY_CPP_TAU_HYBRID_DIR = os.path.join(GILLESPY_PATH, 'c_base/tau_hybrid_cpp_solver')
MAKE_FILE = os.path.dirname(os.path.abspath(
    __file__)) + '/c_base/tau_hybrid_cpp_solver/makefile'
SUNDIALS_DIR = os.path.join(GILLESPY_PATH, 'c_base/Sundials')
CBASE_DIR = os.path.join(GILLESPY_PATH, 'c_base/')


class TauHybridCSolver(GillesPySolver):
    name = "TauHybridCSolver"
    """TODO"""

    def __init__(self, model=None, output_directory=None, delete_directory=True, resume=None, variable=False):
        super(TauHybridCSolver, self).__init__()
    ...
    def __write_template(self):
        ...
        with open(os.path.join(GILLESPY_CPP_TAU_HYBRID_DIR, template_file), 'r') as template:
            ...
            with open(os.path.join(self.output_directory, 'TauHybridSimulation.cpp'), 'w') as outfile:
                for line in template:
                    if line.startswith(template_keyword):
                        ...
                        if line.startswith("REACTIONS"):
                            cutils.write_reactions(
                                outfile, self.model, self.reactions, self.species)
                        if self.variable:
                            if line.startswith("PARAMETER_UPDATES"):
                                cutils.update_parameters(
                                    outfile, self.parameters, self.parameter_mappings)
                    else:
                        outfile.write(line)
    ...
    def __compile(self):
        ...
        try:
            cmd = ["make", "-C", self.output_directory, '-f', MAKE_FILE,
                   'TauHybridSimulation', 'GILLESPY_CPP_TAU_HYBRID_DIR=' +
                   GILLESPY_CPP_TAU_HYBRID_DIR, 'CBASE_DIR='+CBASE_DIR,
    ...
    def run(self=None, model=None, t=20, number_of_trajectories=1, timeout=0,
        ...
        if len(kwargs) > 0:
            for key in kwargs:
                log.warning(
                    'Unsupported keyword argument to {0} solver: {1}'.format(self.name, key))
        ...
        if len(detected_features):
            raise gillespyError.ModelError(
                'Could not run Model.  SBML Feature: {} not supported by TauHybridCSolver.'.format(detected_features))
        ...
        if self.__compiled:
            if self.variable:  # Is a variable simulation
                ...
                parameter_values = cutils.change_param_values(
                    model.listOfParameters, self.parameters, model.volume, variables)
            ...
            args = [os.path.join(self.output_directory, 'TauHybridSimulation'),
                    '-trajectories', str(number_of_trajectories),
                    '-timesteps', str(number_timesteps),
                    '-end', str(t), '-increment', str(increment)]

            if self.variable:
                args.extend(['-initial_values', populations,
