...
class SSACSolver(GillesPySolver):
    ...
    def compile(self):
        # Use makefile.
        cleaned = subprocess.run(["make", "-C", self.output_directory, '--file', 'MakeFile', 'cleanSimulation'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        built = subprocess.run(["make", "-C", self.output_directory, '--file', 'MakeFile', 'UserSimulation'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
